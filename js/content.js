(function($) {
	$(function() {
		...
		$item_list	= $('#content-list ul.sortable');
		$url		= 'admin/pyrocourse/content_order';
		$cookie		= 'open_content';
		...
		data_callback = function(even, ui) {
			...
			$('#content-list ul.sortable').children('li').each(function(){
				root_pages.push($(this).attr('id').replace('content_', ''));
			});
